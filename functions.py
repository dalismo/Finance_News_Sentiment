...
def clean_tableau(x, y, z, w):
    
    df = pd.read_csv(x)
    df.drop(["Unnamed: 0", "words"], axis=1, inplace=True)
    
    df["words"] = df.apply(lambda row: row["text"].split(), axis=1)
    
    df1 = pd.read_csv(y, header=None)
    ...
    xyz = precovid_tableau1[~precovid_tableau1['keywords'].isin(stopwords)]
